...
<%
	If Len(Session.Contents("AccountID")) > 0 Then
		...
		If thisTotalCorrectPicks < (Session.Contents("CurrentPeriod") - 13) Then thisAccountEliminated = 1
		...
		sqlGetSurvivors = "SELECT TotalCorrect, AccountID FROM ( SELECT COUNT(*) AS TotalCorrect, AccountID FROM EliminatorPicks WHERE CorrectPick = 1 AND EliminatorRoundID = " & thisEliminatorRoundID & " GROUP BY AccountID) A WHERE TotalCorrect = " & (Session.Contents("CurrentPeriod") - 13)
